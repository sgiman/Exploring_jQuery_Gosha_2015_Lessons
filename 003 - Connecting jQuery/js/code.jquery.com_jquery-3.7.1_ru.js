...
( function( global, factory ) {
	...
	if ( typeof module === "object" && typeof module.exports === "object" ) {
		...
		// например вар jQuery = require("jquery")(окно);
...
// выдаем исключения, когда нестрогий код (например, ASP.NET 4.5) 
// обращается к строгому режиму аргументы.callee.caller (trac-13335). 
// Но начиная с jQuery 3.0 (2016) строгий режим должен быть обычным явлением.
// достаточно, чтобы все такие попытки были защищены блоком try."use strict";
...
jQuery.extend = jQuery.fn.extend = function() {
	...
	// Обработка случая, когда целью является строка 
	// или что-то в этом роде (возможно при глубоком копировании)
...
jQuery.extend( {
	...
	// Оценивает скрипт в предоставленном контексте; 
	// возвращается к глобальному если не указано.
	...
	text: function( elem ) {
		...
		// Не включайть узлы комментариев или инструкций обработки
	...
	// Поддержка: только Android <= 4.0, только Phantom JS 1
	// push.apply( , arraylike) вызывает древний веб-кит
	...
	map: function (elems, callback, arg) {
		...
		// Проходимся по массиву, переводя каждый из элементов в новые значения
	...
	// jQuery.support не используется в Core, 
	// но другие проекты добавляют свои свойства, 
...
function isArrayLike( obj ) {
	...
	// Поддержка: только настоящая версия i OS 8.2 (не воспроизводится в симуляторе)
	// проверка `in`, используемая для предотвращения ошибки JIT (gh-2145)
	// has Own здесь не используется из-за ложноотрицательных результатов
...
var i,
	...
	pseudos = ":(" + identifier + ")(?:\\((" +
		...
		// Чтобы уменьшить количество селекторов, нуждающихся в токенизации в предварительном фильтре, 
		// отдайте предпочтение аргументам: 1. в кавычках (захват 3; захват 4 или захват 5)
		"('((?:\\\\.|[^\\\\'])*)'|\"((?:\\\\.|[^\\\\\"])*)\")|" +
